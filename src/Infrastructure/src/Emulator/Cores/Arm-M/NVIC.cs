...
namespace Antmicro.Renode.Peripherals.IRQControllers
{
    ...
    public class NVIC : IDoubleWordPeripheral, IKnownSize, IIRQController
    {
        ...
        public uint ReadDoubleWord(long offset)
        {
            ...
            {
                return SetOrClearPendingInterruptRead((int)offset - ClearPendingStart);
            }
            if(offset >= SetPendingStart && offset < SetPendingEnd)
            {
                return SetOrClearPendingInterruptRead((int)offset - SetPendingStart);
            }
        ...
        private uint SetOrClearPendingInterruptRead(int offset)
        {
            lock(irqs)
            {
                var firstIRQNo = 8 * offset + 17;  // 16 is added because this is HW interrupt
                {
                    var returnValue = 0u;
                    ...
                    for(var i = lastIRQNo; i >= firstIRQNo; i--)
                    {
                        this.DebugLog("Value of {0} is {1}", i, irqs[i]);

                        returnValue |= ((irqs[i] & IRQState.Pending) != 0) ? 1u : 0u;
                        returnValue <<= 1;
                    }
                    this.DebugLog("Offset: {0} Return Value: {1}", offset, returnValue);
        ...
        private void SetOrClearPendingInterrupt(int offset, uint value, bool set)
        {
            lock(irqs)
            {
                var firstIRQNo = 8 * offset + 16;  // 16 is added because this is HW interrupt
                {
                    var lastIRQNo = firstIRQNo + 31;
                    var mask = 1u;
                    this.DebugLog("Set bit offset {0}", offset);
                    this.DebugLog("Set bit value {0}", value);
                    for(var i = firstIRQNo; i <= lastIRQNo; i++)
                    {
                        if((value & mask) > 0)
                        {
